...
Gunakan `install.sh` untuk mengunduh dependensi, mengambil repo, build, menerapkan tuning TCP (BBR), lalu menjalankan installer.

```bash
export REPO_URL="https://github.com/your-org/zivpn-installer.git"
export INSTALL_DIR="/opt/zivpn-installer"
sudo ./install.sh
```

Catatan:
- Ubah `REPO_URL` sesuai lokasi repo kamu.
- Tuning TCP menggunakan BBR + FQ dan buffer socket yang lebih besar untuk performa koneksi yang lebih baik.
